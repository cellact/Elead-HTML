...
- **End user**: `chat.html` is home. Native `MAIN` for a lead identity is rewritten to chat and must include `remoteId` (the provider inbox ENS). Empty threads say to write to start the conversation.
...
| `#screen=MAIN&localId=lead-….elead.eth&remoteId=inbox-….elead.eth` | Rewritten to chat |
| `#screen=CHAT&localId=inbox-….elead.eth&sessionId=11` | SP thread (Lead + Approve) |
| `#screen=CHAT&localId=lead-….elead.eth&remoteId=inbox-….elead.eth` | End-user thread |
...
- [In call](http://127.0.0.1:4174/index.html?preview=1#screen=CALL&localId=0xpreview&to=inbox-preview.elead.eth)
