...
export async function loadEnv() {
  ...
  return Object.freeze({
    productLabel: requiredString(merged, 'productLabel', 'AEGIS_PRODUCT_LABEL'),
    productDescription: requiredString(
      merged,
      'productDescription',
      'AEGIS_PRODUCT_DESCRIPTION',
    ),
    productImage:
      typeof merged.productImage === 'string' ? merged.productImage.trim() : '',
    packageType: requiredString(merged, 'packageType', 'AEGIS_PACKAGE_TYPE'),
    sessionRange: requiredCount(merged, 'sessionRange', 'AEGIS_SESSION_RANGE'),
    messageRange: requiredCount(merged, 'messageRange', 'AEGIS_MESSAGE_RANGE'),
    groupMembersUrl:
